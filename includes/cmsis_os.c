...
osMessageQId osMessageCreate(const osMessageQDef_t *queue_def, osThreadId thread_id) {
    (void)thread_id;  // No se usa el thread_id

    if (queue_def == NULL) {
        return NULL;
    }

    // Crear la cola utilizando FreeRTOS
    return (osMessageQId)xQueueCreate(queue_def->queue_sz, sizeof(uint32_t));
}
...
osStatus osMessagePut(osMessageQId queue_id, uint32_t info, uint32_t millisec) {
    ...
    // Enviar un mensaje a la cola con FreeRTOS
    if (xQueueSend((QueueHandle_t)queue_id, &info, pdMS_TO_TICKS(millisec)) == pdPASS) {
        return osOK;
    }

    return osErrorResource;  // La cola está llena
}
...
osEvent osMessageGet(osMessageQId queue_id, uint32_t millisec) {
    ...
    // Recibir un mensaje de la cola con FreeRTOS
    if (xQueueReceive((QueueHandle_t)queue_id, &message, pdMS_TO_TICKS(millisec)) == pdTRUE) {
        event.status = osEventMessage;
        event.value.v = message;
    } else if (millisec == 0) {
        event.status = osErrorResource;  // No hay mensajes disponibles
    } else {
        event.status = osEventTimeout;  // Timeout
    }
